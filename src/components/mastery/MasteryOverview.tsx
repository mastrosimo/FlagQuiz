...
export function MasteryOverview({ masteredCount, total, compact = false }: MasteryOverviewProps) {
  const { t } = useTranslation();
  const ratio = total > 0 ? Math.min(1, masteredCount / total) : 0;
    ...
    <div>
      <div className={`flex items-center justify-between ${compact ? 'text-sm' : 'text-base'}`}>
        ...
        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">
          {t('mastery.percentComplete', { percent })}
        </span>
      </div>
      {!compact && (
        <p className="mt-0.5 text-xs font-medium text-slate-500 dark:text-slate-400">{t('mastery.homeSubtitle')}</p>
      )}
